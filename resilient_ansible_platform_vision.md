...
### 3.4 Explicit Decoupling of Event Stream and Database

A core architectural constraint is that the **event stream and Postgres are completely decoupled**. Concretely:

- Executors **only** depend on the event stream and object storage during execution:
  - They publish events to the event bus without any synchronous interaction with Postgres.
  - They do not perform transactional work that spans “event published” and “row written to DB”.
- The control plane treats Postgres as an **eventual projection** of the event stream:
  - Event consumers read from the stream and *attempt* to write to Postgres.
  - If Postgres is unavailable, consumers stop committing offsets and events accumulate in the stream.
  - When Postgres returns, consumers replay from the last committed offset and rebuild projections.
- Under no circumstances do we couple the liveness of the event bus to Postgres:
  - A DB outage must **not** cause producers to stop sending events to the bus.
  - An event bus outage must **not** cause Postgres writes (for other, unrelated control-plane operations) to block.
- There is **no cross-resource transaction** between the event bus and Postgres:
  - We rely on at-least-once delivery from the stream into Postgres.
  - Idempotency is guaranteed via natural keys such as `(execution_run_id, seq)` and other unique constraints.

This guarantees that the event bus and database can be operated, scaled, and failed over independently, while the system always converges back to a consistent state once both are healthy.
...
The design assumes that the **event stream and Postgres fail and recover independently**. There is no synchronous or transactional dependency between them: producers can continue to publish to the event bus while Postgres is down, and Postgres can continue to serve and mutate control-plane state even if the event stream is degraded or unavailable. All cross-cutting consistency is handled by replayable, idempotent consumers and reconcilers.
