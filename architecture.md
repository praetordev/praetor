...
**Key principles:**  
- Execution must not depend on the real-time availability of the control-plane database.  
- Execution must not depend on the real-time availability of the Event Bus.  

The ultimate source of truth for *what happened* during a run is the executor’s own ordered event/log spool (and its durable copy in object storage). The Event Bus and Postgres are projections over that data used for near-real-time updates and querying.
...
### 1.3 Spool-Centric Execution View (Mermaid)

```mermaid
flowchart LR
    subgraph exec[Executor Pod]
        R[ansible-runner]
        SPOOL[(Local Event/Log Spool)]
    end

    subgraph infra[Backbone]
        EB[(Event Bus)]
        OBJ[(Object Storage)]
        DB[(Postgres)]
    end
    ...
    R -->|TASK_*/JOB_* callbacks| SPOOL
    R -->|stdout| SPOOL

    SPOOL -->|Flush events (best-effort)| EB
    SPOOL -->|Upload logs| OBJ

    EB --> EC
    EC -->|Persist events, log index, summaries| DB

    SPOOL -->|Backfill missing events/log index| REC
    REC -->|Reconcile persisted_event_seq<br/>and final status| DB
```
...
4. For every Ansible callback, appends an ordered record to the local spool (e.g. `/work/runtime/<execution_run_id>/events.jsonl`) and then attempts to publish that event to the Event Bus. If the bus is unavailable, the event remains safely recorded in the local spool for later replay.
5. Streams stdout into the local spool (e.g. `stdout.log`) and, when possible, uploads stdout chunks to object storage and emits corresponding `LOG_CHUNK` events.
6. Periodically emits `HEARTBEAT` events via the Event Bus and also records liveness in the local spool/status file so reconciliation is possible even if the bus is unavailable.
...
2. **Local event spool is the source of truth for execution.**  
   The ordered per-run spool (and its durable copy in object storage) is authoritative; the Event Bus and database are projections over that data. Outages in either only delay visibility, not execution or the ability to reconstruct history.
...
4. **Logs are scalable and resilient.**  
   Executors always write logs/events to a local per-run spool first, then replicate them to object storage and index them in the DB. The UI can stream logs from storage and/or from the spool via reconciliation even if the control plane or Event Bus is partially degraded.
